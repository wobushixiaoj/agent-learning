...
所以 X 的形状 `(3, 4)` 首先只表示：这个矩阵有 3 行、4 列。

然后再解释本例为行列赋予的语义：
- 3 行：每行放一个 Token 的向量，所以本例对应 3 个 Token
- 4 列：每行有 4 个分量，所以本例的每个 Token 使用 4 维向量
- 因此矩阵中共有 3 x 4 = 12 个数
...
`Agent Key = [1, 0, 1, 0]`，也就是 K 的第 1 行。

当前教程先观察一个 Attention head。在一个 head 中，一个 Token 对应一个
Query 向量，所以 `tools` 对应一行；三个 Token 合在一起才组成完整 Q 的三行。
...
先把三种说法分开：
- **严格 Shape：** `Q.shape = (3, 4)` 就是 Q 有 3 行、4 列；
- **本例语义：** 每行放一个 Token 的 Query 向量，所以是 3 个 Query 向量，每个有 4 个分量；
- **标准理论记号：** `Q: (n_q, d_k)`，本例 `n_q=3`、`d_k=4`。

因此，更准确的原句应该是：
> Q 是一个 3 行 4 列的矩阵；在本例中，3 行分别对应 3 个 Token 的 Query 向量，每行有 4 个分量。
...
本例取 `n=3`、`d_k=4`，所以 Q 和 K 都是 3 行 4 列。
