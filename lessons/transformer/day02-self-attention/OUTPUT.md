# Day 2：Self-Attention 如何读取上下文：脚本输出教材
...
```text
========================================================================
Day 2：Self-Attention 如何让 Token 读取上下文
========================================================================
你现在位于整条链路的这个位置：
...
规则 1：左矩阵的一行，与右矩阵的一列，计算出结果中的一个格子。
规则 2：这一行和这一列做点积，即对应位置相乘，再把乘积相加。
规则 3：左矩阵列数必须等于右矩阵行数；结果保留外侧两个数字。

套到本例：
  Q    (3, 4)：左边有 3 行，每行有 4 个数
  K.T  (4, 3)：右边有 3 列，每列有 4 个数
  结果 (3, 3)：3 行分别乘 3 列，一共得到 9 个格子
  中间的 4 和 4 必须相同，因为一次点积需要两组同样多的数。
...
K 的形状也是 (3, 4)。
K.T 表示把 K 转置，所以形状从 (3, 4) 变成 (4, 3)。
这样才能进行矩阵乘法：
  Q (3, 4) @ K.T (4, 3) -> 原始分数 (3, 3)
中间的两个 4 对齐；结果保留外侧的 3 和 3。
最终 3 x 3 表示：3 个读取者分别与 3 个信息来源进行匹配。
...
本节只需要记住：
Self-Attention 让每个 Token 按不同权重读取其他 Token 的信息。
