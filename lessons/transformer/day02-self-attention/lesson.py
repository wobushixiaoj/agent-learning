...
def main() -> None:
    ...
    subsection("5.3 动态直觉：维度像一个会自动收窄的聚光灯")
    print("可以固定两个候选项的真实差异，只把 `d_k` 从 1 慢慢增大。")
    print("随着参与点积的维度增多，未缩放分数的典型幅度会按 `sqrt(d_k)` 放大：")
    print("- 不缩放：Softmax 的聚光灯越来越窄，最后几乎只照到最大项；")
    print("- 除以 `sqrt(d_k)`：抵消维度自带的放大，聚光灯宽度大致不变。")
    print("\n例如令两个输入分数始终是 `[0, sqrt(d_k)]`：")
    print("\n| `d_k` | 不缩放后的 Softmax | 缩放后输入 | 缩放后的 Softmax |")
    print("| ---: | --- | --- | --- |")
    print("| 1 | `[26.9%,73.1%]` | `[0,1]` | `[26.9%,73.1%]` |")
    print("| 16 | `[1.8%,98.2%]` | `[0,1]` | `[26.9%,73.1%]` |")
    print("| 64 | `[0.03%,99.97%]` | `[0,1]` | `[26.9%,73.1%]` |")
